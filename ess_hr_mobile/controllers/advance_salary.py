...
class AdvanceSalaryController(http.Controller):
    ...
    @http.route('/ess/api/advance-salary/cap', type='http', auth='none', methods=['POST'], csrf=False)
    def cap(self):
        kw = get_body()
        employee_id = kw.get('employee_id')
        return call_and_log(
            '/ess/api/advance-salary/cap', employee_id,
            lambda: request.env['hr.advance.salary'].sudo().get_advance_salary_cap(employee_id),
        )

    @http.route('/ess/api/advance-salary/create', type='http', auth='none', methods=['POST'], csrf=False)
    def create(self):
        kw = get_body()
        employee_id = kw.get('employee_id')
        return call_and_log(
            '/ess/api/advance-salary/create', employee_id,
            lambda: request.env['hr.advance.salary'].sudo().create_advance_salary(
                employee_id, kw.get('amount'),
            ),
        )

    @http.route('/ess/api/advance-salary/list', type='http', auth='none', methods=['POST'], csrf=False)
    def list(self):
        kw = get_body()
        employee_id = kw.get('employee_id')
        return call_and_log(
            '/ess/api/advance-salary/list', employee_id,
