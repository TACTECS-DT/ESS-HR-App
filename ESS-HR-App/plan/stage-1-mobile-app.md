...
Build the complete React Native app connected to the real Odoo backend.
All screens and modules are fully functional. The app uses the live Odoo REST API
through the custom `ess_hr_admin` / `ess_hr_client` Odoo modules.
...
## Milestone 2 (API Client & Type Layer)

- Task 2.1 — Install `axios`
- Task 2.2 — Create `src/config/appConfig.ts` with `ACTIVE_BACKEND` flag (`odoo` | `django`)
...
- Task 2.5 — Create TypeScript type definition files for every module under `src/api/types/`:
  - `auth.ts`
  - `attendance.ts`
  - `leave.ts`
  - `payslip.ts`
  ...
  - `experience-certificates.ts`
  - `business-services.ts`
  - `tasks.ts`
  - `timesheets.ts`
  - `profile.ts`
- Task 2.6 — Create `src/api/apiMap.ts` — single source of truth for all endpoint paths
- Task 2.7 — Wire axios client to all module endpoints via `API_MAP`
...
- Task 6.1 — License activation screen: enter server URL, call admin validate endpoint, show error on invalid URL
- Task 6.2 — Company selection screen: list companies returned from server, select and store
...
- Task 15.2 — Service types loaded from API (Odoo configuration)
...
- Task 18.8 — Bilingual error messages for all API error codes
