...
class HrEmployeeExt(models.Model):
    ...
    # ESS mobile credentials — accessible from the employee form ESS Mobile tab
    ess_credential_ids = fields.One2many(
        'ess.employee.credential', 'employee_id', string='ESS Mobile Credentials',
    )
    ...
    def _format_employee_profile(self, employee):
        ...
        return {
            ...
            'birthday': utc_to_date_string(getattr(employee, 'birthday', None)),
    ...
    def _format_contract_summary(self, contract):
        ...
        return {
            'id': contract.id,
            'name': contract.name,
            'date_start': utc_to_date_string(contract.date_start),
            'date_end': utc_to_date_string(contract.date_end),
            'wage': contract.wage,
