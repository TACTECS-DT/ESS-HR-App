...
from ..tz_utils import utc_to_iso_string, wall_clock_to_utc
...
class HrAttendanceExt(models.Model):
    ...
    def get_attendance_status(self, employee_id):
        ...
        check_in_time = False
        if open_att:
            check_in_time = utc_to_iso_string(open_att.check_in)
    ...
    def get_team_attendance(self, manager_employee_id, attendance_date=None):
        ...
        for emp in team:
            ...
            result.append({
                ...
                'status': status,
                'checked_in': bool(open_att),
                'check_in_time': utc_to_iso_string(open_att.check_in) if open_att else False,
                'total_hours': round(total_hours, 2),
                'entries': len(day_records),
            })
        return result
    ...
    @api.model
    def create_manual_attendance(self, employee_id, date_str, check_in_str, check_out_str=None,
                                  latitude=0.0, longitude=0.0, tz_name=None):
        """Create a manual attendance record (HR admin action). Returns attendance dict.

        check_in_str / check_out_str are wall-clock 'HH:MM' (or 'HH:MM:SS') values on
        date_str, in the caller's tz_name -- converted to UTC via wall_clock_to_utc
        since this is the one endpoint where mobile sends a hand-typed time with no
        captured instant.
        """
        employee = self._get_employee(employee_id)
        check_in_dt = wall_clock_to_utc(date_str, check_in_str, tz_name)
        vals = {
            'employee_id': employee_id,
            'check_in': check_in_dt,
            'in_latitude': float(latitude) if latitude else 0.0,
        ...
        if check_out_str:
            vals['check_out'] = wall_clock_to_utc(date_str, check_out_str, tz_name)
        attendance = self.with_user(SUPERUSER_ID).with_context(tracking_disable=True).create(vals)
        return self._format_attendance_record(attendance)
    ...
    def _format_attendance_record(self, att):
        ...
        return {
            ...
            'check_in': utc_to_iso_string(att.check_in),
            'check_out': utc_to_iso_string(att.check_out),
