...
class EssServerStatusWizard(models.TransientModel):
    ...
    def action_apply(self):
        ...
        reason = (self.reason or '').strip() or _('Manual status change by administrator.')
        server.write({'server_status': self.new_status})
        server._log_history(old_status, self.new_status, reason, source='manual')
        ...
        if self.new_status == 'deactivated':
            ...
            if license_rec and license_rec.active:
                license_rec._log_history('active', 'inactive', reason, source='manual')
                license_rec.write({
                    'active': False,
        ...
        # When reactivating, restore the linked license if it was auto-deactivated
            ...
            if license_rec and not license_rec.active:
                license_rec._log_history('inactive', 'active', _('Server reactivated by administrator.'), source='manual')
                license_rec.write({
                    'active': True,
        ...
        if self.action == 'deactivate':
            ...
            for server in license_rec.server_ids:
                if server.server_status != 'deactivated':
                    server._log_history(server.server_status, 'deactivated', reason, source='manual')
            license_rec.server_ids.sudo().write({'server_status': 'deactivated'})

        elif self.action == 'activate':
            if not license_rec.active:
                license_rec._log_history('inactive', 'active', _('Manually activated by administrator.'), source='manual')
            license_rec.write({
                'active': True,
                'deactivation_reason': False,
                'deactivation_date': False,
            ...
            for server in license_rec.server_ids:
                if server.server_status != 'active':
                    server._log_history(server.server_status, 'active', _('License reactivated by administrator.'), source='manual')
