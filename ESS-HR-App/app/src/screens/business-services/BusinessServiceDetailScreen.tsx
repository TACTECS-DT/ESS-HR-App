...
import type {BusinessService} from '../../api/types/business-services';
...
export default function BusinessServiceDetailScreen() {
  ...
  const displayTitle = isAr ? service.title_ar : service.title;
  ...
  return (
    <View style={[styles.container, {backgroundColor: theme.background}]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.typeRow}>
            ...
            <View style={[styles.typeBadge, {backgroundColor: colors.primary + '15'}]}>
              <Text style={[styles.typeBadgeText, {color: colors.primary}]}>
                {isAr ? service.service_type_ar : service.service_type}
              </Text>
            </View>
          </View>
          <InfoRow label={t('businessService.wantedDate')} value={service.requested_date} theme={theme} />
          <InfoRow label={t('common.date')} value={service.request_date} theme={theme} />
          {service.reason ? (
            <>
              <View style={[styles.divider, {borderColor: theme.border}]} />
              <Text style={[styles.reasonLabel, {color: theme.textSecondary}]}>{t('businessService.reason')}:</Text>
              <Text style={[styles.reasonText, {color: theme.text}]}>
                {isAr ? (service.reason_ar ?? service.reason) : service.reason}
              </Text>
            </>
          ) : null}
        </View>
        ...
        {/* Approval Timeline */}
        {service.approval_history.length > 0 ? (
          <>
            <Text style={[styles.sectionTitle, {color: theme.text}]}>{t('leave.approvalHistory')}</Text>
            <View style={[styles.card, {backgroundColor: theme.surface, borderColor: theme.border}]}>
              {service.approval_history.map((step, idx) => {
                const dotColor = stepDotColor(step.status);
                const isLast = idx === service.approval_history.length - 1;
                return (
...
const styles = StyleSheet.create({
  ...
  serviceTitle: {fontSize: fontSize.lg, fontWeight: '700', flex: 1, marginRight: spacing.sm},
