...
import {formatLocalDate, splitLocalDateTime} from '../../utils/timezone';

type FilterPeriod = 'this_month' | 'last_month' | 'custom';

function formatDate(dateStr: string, locale: string): string {
  return formatLocalDate(dateStr, locale, {weekday: 'short', month: 'short', day: 'numeric'});
}
  ...
  const {data, isLoading, refetch} = useQuery({
    ...
    queryFn: async () => {
      ...
      return records.map((r): AttendanceRecord => {
        ...
        const {date: checkInDate, time: checkInTime} = splitLocalDateTime(checkInDt);
        const {time: checkOutTime} = splitLocalDateTime(checkOutDt);
        const dateStr: string = checkInDate ?? (r.date ?? '');
        return {
          id: r.id,
          date: dateStr,
