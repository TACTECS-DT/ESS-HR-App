...
export default function AttendanceDashboardScreen() {
  ...
  function showFeedbackModal(
    ...
    setFeedbackModal({visible: true, type, title, message, time});
  }
  ...
  // Refetch whenever this screen gains focus — picks up changes from physical terminals
  useFocusEffect(
    useCallback(() => {
      refetch();
    }, [refetch]),
  ...
  const checkInMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      if (res?.data?.already_checked_in) {
        ...
        showFeedbackModal(
          'checkin',
          t('attendance.checkInSuccess', 'Checked In!'),
          undefined,
          formatLocalTime(res?.data?.check_in, i18n.language),
        );
      }
    },
    onError: (err: any) => {
  ...
  const checkOutMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      showFeedbackModal(
        'checkout',
        t('attendance.checkOutSuccess', 'Checked Out!'),
        undefined,
        formatLocalTime(res?.data?.check_out, i18n.language),
      );
    },
    onError: (err: any) => {
      if (err?.message === 'location_permission_denied') {
  ...
  const checkInTimeFmt = summary?.check_in_time
    ? formatLocalTime(summary.check_in_time, i18n.language)
    : null;
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {(feedbackModal.type === 'checkin' || feedbackModal.type === 'checkout') && feedbackModal.time ? (
              <Text style={styles.modalTimeText}>{feedbackModal.time}</Text>
            ) : null}
            <TouchableOpacity
              style={[
