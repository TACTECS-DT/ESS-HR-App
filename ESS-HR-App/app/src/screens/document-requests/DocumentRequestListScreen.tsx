...
import type {DocumentRequest} from '../../api/types/document-requests';
import {API_MAP} from '../../api/apiMap';

type Nav = StackNavigationProp<RequestsStackParamList>;
type Filter = 'all' | 'draft' | 'my_requests';

export default function DocumentRequestListScreen() {
  ...
  return (
    <View style={[styles.container, {backgroundColor: theme.background}]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            actionLabel={t('documentRequest.request')}
            onAction={() => navigation.navigate('DocumentRequestCreate')}
          />
        }
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('DocumentRequestDetail', {id: item.id})}>
            <View style={styles.cardTop}>
              <Text style={[styles.docType, {color: theme.text}]} numberOfLines={1}>
                {isAr ? item.document_type_ar : item.document_type}
              </Text>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('DocumentRequestCreate')}>
