...
def _gather_pending(req, approver_employee_id):
    """Aggregate pending approval items across all request modules for the approver."""
    result = []
    env = req.env

    # Leaves pending first approval (state=confirm) where manager matches
    ...
    # Experience certificates submitted
    ...
    # Business service requests submitted
...
def _dispatch_action(req, item_id, item_type, action_name, approver_id, reason):
    """Dispatch an approve/refuse action to the correct model."""
    env = req.env
    _type_map = {
