...
class HrLeaveExt(models.Model):
    ...
    def get_leave_balance(self, employee_id):
        ...
        leave_types = self.env['hr.leave.type'].with_user(SUPERUSER_ID).search([('active', '=', True)])
        result = []
        for lt in leave_types:
    ...
    def create_leave_request(self, employee_id, leave_type_id, date_from, date_to,
        ...
        leave = self.with_user(SUPERUSER_ID).with_context(mail_notify_force_send=False).create(vals)
    ...
    def refuse_leave(self, leave_id, manager_employee_id, reason):
        ...
        leave.with_user(SUPERUSER_ID).action_refuse()
        if reason:
            leave.with_user(SUPERUSER_ID).write({'name': (leave.name or '') + ' [Refused: %s]' % reason})
        return True
    ...
    def get_team_leave_allocations(self, manager_employee_id):
        ...
        team_members = self.env['hr.employee'].with_user(SUPERUSER_ID).search([
