...
export default function AdvanceSalaryDetailScreen() {
    ...
    <View style={[styles.container, {backgroundColor: theme.background}]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={[styles.card, {backgroundColor: theme.surface, borderColor: theme.border}]}>
          ...
          {/* Large amount */}
          <Text style={[styles.amountLarge, {color: colors.primary}]}>
          ...
          {item.reason ? (
            <>
              <View style={[styles.divider, {borderColor: theme.border}]} />
              <Text style={[styles.reasonLabel, {color: theme.textSecondary}]}>{t('advanceSalary.reason')}:</Text>
        ...
        {item.approval_history.length > 0 ? (
          <>
            ...
            <View style={[styles.card, {backgroundColor: theme.surface, borderColor: theme.border}]}>
              {item.approval_history.map((step, idx) => {
                const dotColor = stepDotColor(step.status);
                const isLast = idx === item.approval_history.length - 1;
                return (
...
const styles = StyleSheet.create({
  ...
  itemTitle: {fontSize: fontSize.lg, fontWeight: '700', flex: 1, marginRight: spacing.sm},
  amountLarge: {fontSize: 32, fontWeight: '700'},
